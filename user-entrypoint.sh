...
if [ -n "${USER_ID:-}" ] && [ -n "${GROUP_ID:-}" ]; then
    ...
    groupmod -g "$GROUP_ID" metplus_user 2>/dev/null || \
        groupadd -g "$GROUP_ID" metplus_user 2>/dev/null || true

    # Modify user
    usermod -u "$USER_ID" -g "$GROUP_ID" metplus_user 2>/dev/null || true

    # Fix ownership of directories that metplus_user needs to write to
    chown -R metplus_user:metplus_user /home/metplus_user 2>/dev/null || true
